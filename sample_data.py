...
def insert_sales(connection, num_sales=1000):
    ...
    for _ in range(num_sales):
        ...
        sale_timestamp = start_date + timedelta(days=random_days, hours=random_hours, minutes=random_minutes)
        ...
        # Price might vary slightly from base price (discounts/promotions)
...
def populate_dimensional_model(connection):
    """Populate star-schema dimensions and fact table from OLTP tables."""
    ...
    connection.execute(text("""
        ...
        SELECT
            CAST(DATE_FORMAT(sale_date, '%Y%m%d') AS UNSIGNED) AS date_key,
        ...
        FROM (
            SELECT DISTINCT DATE(sale_timestamp) AS sale_date
            FROM sales
        ) dates
    """))
